...
Route.group(() => {
  ...
  Route.get('tran', 'TranController.index').middleware('auth');
  Route.get('tran2', 'TranController.translateText').middleware('auth');
  Route.post('messages', 'TranController.postMessages').middleware('auth');
  Route.post('suggest', 'TranController.suggest').middleware('auth');
...
if (Env.get('IS_DASHBOARD_ENABLED') !== 'false') {
  Route.group(() => {
    ...
    Route.get('/user/:userId/services', 'AdminController.userServices');
    Route.get('/user/:userId/service/:serviceId/messages', 'AdminController.userServiceMessages');
    Route.get('/user/:userId/service/:serviceId/:peerId/messages', 'AdminController.userServicePeerMessages');
    Route.post('/user_charge', 'AdminController.userCharge');
...
// 404 handler
Route.get('/*', ({ response }) => response.notFound());
