...
class RecipeController {
  ...
  async popularRecipes({
    ...
    const featuredFile = path.join(Helpers.appRoot(), 'recipes', 'featured.json');
    const featuredRecipes = await fs.readJSON(featuredFile);
    const allRecipes = await fs
      .readJSON(path.join(
        Helpers.appRoot(), 'recipes', 'all.json',
      ));
    return response.send(
      featuredRecipes.map((featuredRecipeId) => allRecipes.find((_recipe) => _recipe.id === featuredRecipeId)),
    );
  }
  ...
  update({ request, response }) {
    ...
    console.log('recipe update', updates);
